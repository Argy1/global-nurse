...
export function useSEO() {
  ...
  useEffect(() => {
    ...
    // Skip dynamic article routes — handled by useArticleSEO in the page itself
    if (
      pathname.match(/^\/news\/.+/) ||
      pathname.match(/^\/success-stories\/.+/)
    ) {
      return;
    }
...
/**
 * useArticleSEO — call inside NewsDetail / SuccessStoryDetail once article data is loaded.
 * Falls back to route-level SEO until data arrives.
 */
